...
/*
 * Heterogeneous A100 worker.
 *
 * Assign complete 32-column macro-panels to workers.  Each full 16x32
 * region uses the exact validated rm=2,rn=4 IME kernel.  Smaller row/column
 * tails fall back to the already-existing 8x16 / 8x8 / scalar kernels.
 */
                          ...
                          int worker_id)
{
  ...
  const long PANEL_N = 32;
  const long np = (col_end - col_begin + PANEL_N - 1) / PANEL_N;
  const long p0 = (np * worker_id) / workers;
  const long p1 = (np * (worker_id + 1)) / workers;

  for (long p = p0; p < p1; ++p) {
    const long col = col_begin + p * PANEL_N;
    const long cols = MIN(PANEL_N, col_end - col);

    long row = 0;

    if (cols == 32) {
      for (; row + 16 <= order; row += 16)
        ime_tile_16x32(A, B, C, order, row, col);
    }

    /*
     * Handle a partial 32-column panel, or a remaining row tail, using
     * the smaller validated IME tiles where possible.
     */
    for (; row < order; ) {
      const long rows_left = order - row;
      long cj = 0;

      if (rows_left >= 8) {
        for (; cj + 16 <= cols; cj += 16)
          ime_tile_8x16(A, B, C, order, row, col + cj);
        for (; cj + 8 <= cols; cj += 8)
          ime_tile_8x8(A, B, C, order, row, col + cj);
        if (cj < cols)
          scalar_edge(A, B, C, order, row, col + cj,
                      MIN(8L, rows_left), cols - cj);
        row += 8;
      } else {
        scalar_edge(A, B, C, order, row, col, rows_left, cols);
        row = order;
      }
    }
  }
